...
#+begin_src emacs-lisp
  ...
  ;; (when window-system (add-hook 'prog-mode-hook 'hl-line-mode)) ;; hightlight current line (but only in GUI mode)
#+end_src
** Non GUI
...
#+begin_src emacs-lisp
    (use-package projectile
      :ensure t
      :config
      (setq projectile-require-project-root nil) ;; this line will allow for projectile commands to be run even outside of any project
      (define-key projectile-mode-map (kbd "C-c p") 'projectile-command-map) ;; counsel-projet uses this command
      (projectile-mode 1))
#+end_src
*** Swiper
